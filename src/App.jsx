...
function LoginScreen({ onLogin, error }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100vh', background: 'linear-gradient(135deg, #0D0D1A 0%, #16213E 100%)' }}>
      <div style={{ background: '#1A1A2E', border: '2px solid #FFD700', borderRadius: '12px', padding: '40px', maxWidth: '400px', width: '90%', boxShadow: '0 8px 32px rgba(255,215,0,0.2)' }}>
        <h1 style={{ fontFamily: "'Bebas Neue'", fontSize: '36px', color: '#FFD700', marginBottom: '10px', letterSpacing: '2px', textAlign: 'center' }}>POKÉDEX</h1>
        <p style={{ color: '#888', textAlign: 'center', marginBottom: '30px', fontSize: '14px' }}>Écarlate Neo — Collection Privée</p>
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', color: '#F0F0F0', marginBottom: '8px', fontWeight: '600', fontSize: '14px' }}>Email</label>
            <input
              type="email"
              value={email}
              ...
              required
              autoFocus
              style={{ width: '100%', padding: '12px', background: '#0D0D1A', border: '1px solid #FFD700', borderRadius: '6px', color: '#F0F0F0', fontSize: '14px' }}
            />
          </div>
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', color: '#F0F0F0', marginBottom: '8px', fontWeight: '600', fontSize: '14px' }}>Mot de passe</label>
            <input
              type="password"
              value={password}
          ...
          <button
            type="submit"
            style={{ width: '100%', padding: '12px', background: '#FFD700', border: 'none', borderRadius: '6px', color: '#0D0D1A', fontWeight: '700', fontSize: '16px', cursor: 'pointer', transition: '0.3s' }}
          >
            Accès
          </button>
          {error && <p style={{ color: '#FF5252', fontSize: '13px', marginTop: '10px', textAlign: 'center' }}>{error}</p>}
        </form>
      </div>
    </div>
  );
...
function PokemonRegistry() {
  ...
  const loadStorageData = useCallback(async () => {
    ...
    try {
      const [collResult, notesResult] = await Promise.all([
        supabase.from('collections').select('set_id, card_number, status').eq('user_id', user.id),
        supabase.from('card_notes').select('card_id, condition, price, purchase_date, note_text').eq('user_id', user.id)
      ]);

      const ownedData = {};
      for (const row of (collResult.data ?? [])) {
        if (!ownedData[row.set_id]) ownedData[row.set_id] = {};
      ...
      for (const row of (notesResult.data ?? [])) {
        notesData[row.card_id] = { condition: row.condition, price: row.price, date: row.purchase_date, text: row.note_text };
      }
      setNotes(notesData);
    } catch (e) {
  ...
  const cycleCardStatus = useCallback((setId, cardNumber) => {
    setOwned(prev => {
      const current = prev[setId]?.[cardNumber];
      ...
      if (user) {
        if (next === undefined) {
          supabase.from('collections').delete()
            ...
            .then(({ error }) => error && console.error('Delete error:', error));
        } else {
          supabase.from('collections').upsert({
            user_id: user.id,
            set_id: setId,
            card_number: String(cardNumber),
            status: next === true ? 'owned' : 'wish',
      ...
      return { ...prev, [setId]: { ...(prev[setId] || {}), [cardNumber]: next } };
    });
  }, [user]);
    ...
    fileInput.onchange = async (e) => {
      ...
      try {
        ...
        if (data.owned && typeof data.owned === "object") {
          ...
          if (user) {
            const rows = [];
            for (const [setId, cardMap] of Object.entries(data.owned)) {
              for (const [cardNumber, status] of Object.entries(cardMap)) {
                if (status === true || status === 'wish') {
                  rows.push({
                    user_id: user.id,
                    set_id: setId,
                    card_number: String(cardNumber),
                    status: status === true ? 'owned' : 'wish',
                    updated_at: new Date().toISOString()
                  });
                }
              }
            ...
            for (let i = 0; i < rows.length; i += 500) {
              const { error } = await supabase.from('collections')
                .upsert(rows.slice(i, i + 500), { onConflict: 'user_id,set_id,card_number' });
  ...
  const handleUpdateNote = useCallback(async (cardId, noteData) => {
    setNotes(prev => ({ ...prev, [cardId]: noteData }));
    if (user) {
      const { error } = await supabase.from('card_notes').upsert({
        user_id: user.id,
        card_id: cardId,
        condition: noteData.condition || null,
        price: noteData.price || null,
        purchase_date: noteData.date || null,
        note_text: noteData.text || null,
        updated_at: new Date().toISOString()
  ...
  if (authLoading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#0D0D1A", color: "#888" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: "48px", marginBottom: "20px" }}>🎴</div>
          <div style={{ fontSize: "18px" }}>Initialisation...</div>
        </div>
      </div>
    );
  }
